...
def main(outputFolder) {
    ...
    goThroughProjects(outputFolder)
}
...
def goThroughProjects(outputFolder) {
    projects.each { project ->
        if (projectNames.contains(project.name)) {
            project.testSuites.each { testSuite ->
                if (!ignoreSuites.contains(testSuite.name)) {
                    testSuite.testCases.each { testCase ->
                        def testCaseData = getTestCaseData(testCase)
                        def outputFile = new File(outputFolder, "${project.name}-${testSuite.name}-${testCase.name}.json")
                        outputFile.text = JsonOutput.prettyPrint(JsonOutput.toJson(testCaseData))
                        log.info("JSON export written to: ${outputFile.getPath()}")
                    }
                }
            }
        }
...
def getTestCaseData(testCase) {
    def steps = testCase.testSteps.collect { testStep ->
        def request = //testStep.getHttpRequest()
        [
            testStepName : 'stepName' //testStep.name,
            method       : 'GET' //request.method.toString(),
            url          : 'url' //request.path,
            pathParams   : 'pathParams' //request.getParams(),
            headers      : 'headers' //request.getRequestHeaders(),
            requestBody  : 'requestBody' //request.getRequestContent(),
